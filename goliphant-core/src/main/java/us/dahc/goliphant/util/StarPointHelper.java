...
public class StarPointHelper {
    ...
    private static List<Vertex> getStarPoints(Board board, int edgeDistance, int total) {
        ...
        starPoints.add(Vertex.get(edgeDistance - 1, edgeDistance - 1));                                 // D4
        starPoints.add(Vertex.get(board.getRows() - edgeDistance, board.getColumns() - edgeDistance));  // Q16
        starPoints.add(Vertex.get(board.getRows() - edgeDistance, edgeDistance - 1));                   // D16
        starPoints.add(Vertex.get(edgeDistance - 1, board.getColumns() - edgeDistance));                // Q4
        if (total > 5) {
            starPoints.add(Vertex.get(board.getRows() / 2, edgeDistance - 1));                          // D10
            starPoints.add(Vertex.get(board.getRows() / 2, board.getColumns() - edgeDistance));         // Q10
        }
        if (total > 7) {
            starPoints.add(Vertex.get(edgeDistance - 1, board.getColumns() / 2));                       // K4
            starPoints.add(Vertex.get(board.getRows() - edgeDistance, board.getColumns() / 2));         // K16
        }
        starPoints.add(Vertex.get(board.getRows() / 2, board.getColumns() / 2));                        // K10
        return starPoints;
    }
